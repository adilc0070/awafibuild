...
exports.SalesReportExcelGenerator = exports.AdminSalesRepository = void 0;
const checkoutModel_1 = require("../model/checkoutModel");
const exceljs_1 = __importDefault(require("exceljs"));
const moment_1 = __importDefault(require("moment"));
const constants_1 = require("../../utils/constants");
class AdminSalesRepository {
    getDateRangeFilter(reportType, startDate, endDate) {
        const now = new Date();
        let start, end;
        switch (reportType) {
            case 'weekly':
                start = (0, moment_1.default)(now).subtract(7, 'days').toDate();
                end = now;
                break;
            case 'monthly':
                start = (0, moment_1.default)(now).subtract(1, 'months').toDate();
                end = now;
                break;
            case 'yearly':
                start = (0, moment_1.default)(now).subtract(1, 'years').toDate();
                end = now;
                break;
            case 'custom':
    ...
    async generateSalesReport(reportType, startDate, endDate) {
        ...
        const salesData = await checkoutModel_1.CheckoutModel.aggregate([
            {
                $match: {
                    orderPlacedAt: {
                        $gte: start,
        ...
        const totalRevenueResult = await checkoutModel_1.CheckoutModel.aggregate([
            {
                $match: {
                    orderPlacedAt: {
                        $gte: start,
    ...
    async exportSalesReportToExcel(reportType, startDate, endDate) {
...
exports.AdminSalesRepository = AdminSalesRepository;
class SalesReportExcelGenerator {
    ...
    addCompanyLogo() {
        const image = this.workbook.addImage({
            base64: constants_1.LOGO_BASE64,
            extension: 'png',
        });
...
exports.SalesReportExcelGenerator = SalesReportExcelGenerator;
//# sourceMappingURL=adminSalesRepo.js.map